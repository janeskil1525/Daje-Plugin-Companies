...
sub load_all_companies_users($self) {
    ...
    $self->v_companies_users_list->load_all_companies_users_p($companies_pkey, $users_pkey)->then(sub($result) {
...
sub load_list_companies_companies_fkey($self) {
    ...
    $self->app->log->debug('Daje::Controller::Super::CompaniesUsersList::load_listauthorities_role_fkey ');
    $self->render_later;
    my ($companies_pkey, $users_pkey) = $self->jwt->companies_users_pkey(
         $self->req->headers->header('X-Token-Check')
    );
    ...
    $self->app->log->debug($self->req->headers->header('X-Token-Check'));
    # my $setting = $self->param('setting');
    $self->v_authorities__list->authorities_role_fkey_p($companies_pkey, $users_pkey,$fkey)->then(sub($result) {
        $self->render(json => $result->{data});
    })->catch(sub($err) {
        $self->app->log->error('Daje::Controller::Super::CompaniesUsers::load_authorities_role_fkey ' . $err);
        $self->render
    });
}
1;
